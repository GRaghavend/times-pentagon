...
# SARSA is ON-POLICY TD CONTROL: it learns Q(s,a) using the action the
# epsilon-greedy policy ACTUALLY takes next, so the policy being learned and
# the policy generating behavior are the same one. Name comes from the
# quintuple it consumes each step: State, Action, Reward, State', Action'.
...
for episode_number in range(NUM_EPISODES):
    ...
    action = epsilon_greedy_action(state, epsilon)   # pick A before the loop starts
    ...
    while True:
        ...
        next_action = epsilon_greedy_action(next_state, epsilon)   # A' from the SAME epsilon-greedy policy

        # ---- SARSA update ----
        # Target bootstraps off Q(s', a') - the action actually chosen next,
        # not the max over next-state actions. This is what makes it on-policy.
        Q[state][action] = Q[state][action] + ALPHA * (
            reward + GAMMA * Q[next_state][next_action] - Q[state][action]
        )

        state, action = next_state, next_action   # carry A' forward as next step's A
